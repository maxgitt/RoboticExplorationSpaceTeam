...
Odometry::detectRoverBeacons(){
    ...
    getline(cin,beaconID); //consume useless first line
    timer.recordTime();
...
void 
Odometry::loadBiases(){
    cout << "Please place the rover (center of the rover) at position (0,2) with the center of the sieve being (0,0)\n";
    timer.recordTime();
    double startTime = timer.getTime(); //1 = 20 seconds
    double time_difference = 0;


    while (time_difference < 3) { //85 = wait for ~8 minutes
        timer.recordTime();
        time_difference = timer.getTime() - startTime;
	/*if ((int)time_difference % 12 == 0){
		cout << "You have approx. " << (int)time_difference << " minutes left to go" << endl;
	}*/
    }
    ...
    for (; roverBeaconIt != endRoverBeaconIt; ++roverBeaconIt) {
        ...
        double rBeaconYPos = roverBeaconIt->getOffset().second + 2;
        unordered_map<int, RoverBeacon::sieveBeaconData>::iterator sieveIt = roverBeaconIt->beaconReadings.begin();
        unordered_map<int, RoverBeacon::sieveBeaconData>::iterator endSieveIt = roverBeaconIt->beaconReadings.end();
        for (; sieveIt != endSieveIt; ++sieveIt) {
            double sieveXPos = sieveIt->second.offset.first;
            double sieveYPos = sieveIt->second.offset.second;
            double expectedValue = sqrt(pow((rBeaconXPos - sieveXPos),2) + pow((rBeaconYPos - sieveYPos), 2));
    ...
    cout << "All rover beacons have been fully configured. We are ready to output beautiful odometry data!\n";
        ...
        for (; roverBeaconIt != endRoverBeaconIt; ++roverBeaconIt) {
           auto sieveIt = roverBeaconIt->beaconReadings.begin();
           auto endSieveIt = roverBeaconIt->beaconReadings.end();
            ...
            for (; sieveIt != endSieveIt; ++sieveIt) {
                cout << "     " << sieveIt->first << " has an offset of " << sieveIt->second.offset.first << ","
                << sieveIt->second.offset.second << " and a bias of " << sieveIt->second.bias << "\n";
            }
        }
#endif
