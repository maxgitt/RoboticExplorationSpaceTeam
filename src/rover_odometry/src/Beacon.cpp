...
RoverBeacon::RoverBeacon(string id_, 
			vector<std::pair<int, sieveBeaconData>> sieveBeacons, 
				pair<double,double> roverOffset) : id(id_), 
					offset(roverOffset) {
	//loop through the sieveBeacon vector to intialize the rover beacon;
	for(auto beacon: sieveBeacons){ 
		//store the id of the sieve beacon and its associated data
